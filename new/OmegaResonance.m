function OmegaRe=OmegaResonance(n,order_max,cs_ca,L_a,sigma_a_rho, ...
...
if n==0% ii从0开始编号
    ...
    Omega_loop=0:0.01:50;% as the paper
...
transMat=zeros(length(OmegaRe),order_max);% 膜腔之间的变换矩阵
KtransMat=zeros(length(OmegaRe),order_max,order_max);% 保存K矩阵
tol0=0.1;
for ii =1: length(OmegaRe)
    mat_temp=eye(1,order_max)+mat_temp;
    transMat_temp=[];
    while size(transMat_temp,2)~=1
        transMat_temp=null2022(mat_temp,tol0);
        tol0=tol0/1.1;% 魔法数字
        if size(transMat_temp,2)==0
            tol0=tol0*2;% 魔法数字
            disp(['warning' num2str(num2str(clock))])
        end
    end
    tol0=tol0*2;% 魔法数字
    transMat(ii,:)=transMat_temp';
    %      计算并保存变换矩阵
    Omega=OmegaRe(ii);
    for jj=1:order_max
        sqrt_tmp=sqrt(abs(rootBesselDiff(n+1,jj+1)^2-Omega^2));
        if Omega<rootBesselDiff(n+1,jj+1)
            K(jj,jj)=-Omega^2/sqrt_tmp*coth(L_a*sqrt_tmp);% gamma*p_a/a can be eliminate
        elseif Omega>rootBesselDiff(n+1,jj+1)
            K(jj,jj)=Omega^2/sqrt_tmp*cot(L_a*sqrt_tmp);
        else
            K(jj,jj)=inf;
        end
    end
...
transMat=transMat';% 便于projection中直接乘
save(sprintf('OmegaRe/%d.mat',n),'OmegaRe');
save(sprintf('TransMat/%d.mat',n),'transMat');
save(sprintf('K/%d.mat',n),'KtransMat'); %K(order,ni,mi)
end