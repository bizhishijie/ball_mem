...
%%
x_length=10000; % 横轴划分的个数
omega_limit=10;
t=linspace(0,1,x_length);
y=zeros(1,x_length);
for nn=0:order_max
    for mm=1:order_max
        y=y+w_cn(nn+1,mm)*cos(Omega(nn+1,mm)*ca/a*t);
    end
end
Y = fft(y);
Y=abs(Y);
Y=Y(1:end/2);
plot(0:(x_length/2-1),Y)
xlim([0,500])
...
for nn=0:order_max
    for mm=1:order_max
        y=y+((abs(x-Omega(nn+1,mm)*ca/a/2/pi)<omega_limit*ca/a/fn/2/pi))*1000;
    end
end
y(y>1000)=1000;
